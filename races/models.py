...
class Race(models.Model):
    ...
    def get_absolute_url(self):
        return reverse(
            "races:detail", kwargs={"year": self.season_start_year, "slug": self.slug}
        )
    ...
    def calculate_positions(self):
        ...
        for i, result in enumerate(
            (res for res in results if res.time is not None), start=1
        ):
        ...
        for gender_prefix in ["M", "F", "N"]:
            ...
            for i, result in enumerate(
                (res for res in gender_results if res.time is not None), start=1
            ):
