...
class RaceModelTest(TestCase):
    ...
    def setUpTestData(cls):
        Race.objects.create(name='Test Race', description='Test Description', park='Test Park',
                            coordinates='55.82,-4.26', race_date=date.today())
...
class RunnerModelTest(TestCase):
    ...
    def test_str(self):
        runner = Runner.objects.get(id=1)
        expected_object_name = f'{runner.first_name} {runner.middle_name} {runner.last_name}'
...
class ResultModelTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        race = Race.objects.create(name='Test Race', description='Test Race Description', park='Test Park',
                                   coordinates='40.748817,-73.985428', race_date=date.today(),
                                   race_file=SimpleUploadedFile('test_file.txt', b'This is a test file'))
        runner = Runner.objects.create(
            first_name='Test', last_name='Runner', category='M50')
        Result.objects.create(race=race, runner=runner,
                              time=timedelta(hours=1, minutes=30, seconds=15))
